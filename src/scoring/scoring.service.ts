...
export class ScoringService {
  ...
  private async calculateEventStats(
    ...
    // BUGFIX: isRealiableStats é async; sem await a condição (Promise) era
    // sempre verdadeira → nunca caía no defaultNeutralScore e podia gerar
    // NaN (0/0) quando não havia avaliações. Ver ROADMAP-EVOLUCAO.md.
    const publicMeanScore = (await this.isRealiableStats(
      eventEditionId,
    ...
    const panelistMeanScore = (await this.isRealiableStats(
      eventEditionId,
    ...
    const meanScore = (await this.isRealiableStats(
      eventEditionId,
      totalEvaluations,
    ))

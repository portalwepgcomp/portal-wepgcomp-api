...
export class ScoringCalculatorService {
  ...
  async calculateEventStats(eventEditionId: string): Promise<EventStats> {
    ...
    const publicMeanScore = (await this.isReliableStats(
      eventEditionId,
    ...
    const panelistMeanScore = (await this.isReliableStats(
      eventEditionId,
    ...
    const meanScore = (await this.isReliableStats(
      eventEditionId,
      totalEvaluations,
    ))
